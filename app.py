...
def run_premium_engine(df):
    ...
    # Run optimal K=4 based on Silhouette studies
    k_opt = 4
    kmeans = KMeans(n_clusters=k_opt, random_state=42, n_init=10)
    df['cluster'] = kmeans.fit_predict(X_scaled)
    
    cluster_mapping = {0: 'Chill Acoustic', 1: 'Party Tracks', 2: 'Vocal / Live', 3: 'Instrumental'}
...
try:
    ...
    met_2.metric("Silhouette Score (k=4)", f"{silhouette_score_k4:.3f}")
    met_3.metric("Algorithm", "K-Means++")
    met_4.metric("Musical Categories", "4")
    ...
    filter_cat = st.sidebar.selectbox("Filter  Dashboard", 
                                     options=['All Categories', 'Chill Acoustic', 'Party Tracks', 'Vocal / Live', 'Instrumental'])
